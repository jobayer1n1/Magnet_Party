...
def console():
        # Get the watchlist
        from utils.letterboxd_tools import LetterboxdLoginManager
        login_manager = LetterboxdLoginManager("logins")

        from utils.terminal_tools import clearScreen

        from utils.letterboxd_tools import get_watchlist
        while True:
            clearScreen()
            ...
            if watchlist:
                ...
                while True:
                    ...
                    if choice == "b":
                        return
                            ...
                            from utils.letterboxd_tools import retrieve_letterboxd_data
                            success = retrieve_letterboxd_data(login_manager)
                            if success:
                                console()
                                return
                            else:
                                print("Incomplete load watchlist request.\n")
                                time.sleep(1)
                                console()
                                return
                ...
                if not movies:
                    ...
                    while True:
                        print("Your watchlist is empty\n\nEnter load letterboxd watchlist or llw to retrieve your letterboxd watchlist or b to go back : ")
                        choice = input("main->letterboxd watchlist > ")
                        if choice == "b":
                            return
