...
def add(MAGNET=None):
    ...
    from utils.webtorrent_tools import getStreamableLink,getVideoLinks
...
def stream(number):
    number = int(number)
    from utils.webtorrent_tools import retrieveMagnetLink
    magnetLink = retrieveMagnetLink(number=number)
    if not magnetLink:
        print("Invalid selection or missing links.")
        return
    from utils.media_player_tools import get_player_name
